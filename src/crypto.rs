...
impl std::fmt::Display for CryptoError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            CryptoError::MemoryLockError => write!(f, "Memory lock failed (check OS limits)"),
        }
    }
}
...
impl ProtectedBuffer {
    pub fn new(mut data: Vec<u8>) -> Result<Self, CryptoError> {
        ...
        {
            let ret = unsafe { windows_sys::Win32::System::Memory::VirtualLock(ptr, len) };
            if ret == 0 {
                data.zeroize();
                return Err(CryptoError::MemoryLockError);
            }
        }
        Ok(ProtectedBuffer { data })
    }
...
impl Drop for ProtectedBuffer {
    fn drop(&mut self) {
        self.data.zeroize();
        unsafe {
            let ptr = self.data.as_mut_ptr() as *mut std::ffi::c_void;
...
fn rayo_expand_key(argon2_output: &[u8; KEY_LEN], steps: u64) -> ProtectedBuffer {
    ...
    let h = Hkdf::<Sha256>::new(Some(b"alekscypher-v3"), &ikm);
    let mut ka = vec![0u8; KEY_LEN];
    h.expand(b"aes-key", &mut ka).expect("HKDF expand aes-key");
    ProtectedBuffer::new(ka).expect("ProtectedBuffer creation")
}

pub fn derive_keys(password: &[u8], salt: &[u8], rayo_steps: u64) -> Result<ProtectedBuffer, CryptoError> {
    ...
    let mut ikm = vec![0u8; KEY_LEN];
    argon2.hash_password_into(password, salt, &mut ikm).map_err(|e| CryptoError::Argon2Error(e.to_string()))?;
    let aes = rayo_expand_key(&ikm[..KEY_LEN].try_into().expect("Key length mismatch"), rayo_steps);
    ikm.zeroize();
    Ok(aes)
}

pub fn encrypt_file(
    input_path: &Path, output_path: &Path, password: ProtectedBuffer,
    padding_level: u8, rayo_steps: u64,
) -> Result<(), CryptoError> {
    ...
    let key_enc = derive_keys(password.as_slice(), &salt, rayo_steps)?;
    let cipher = Aes256Gcm::new_from_slice(key_enc.as_slice()).map_err(|_| CryptoError::DecryptionError)?;

    let mut plaintext = fs::read(input_path)?;
    let original_len = plaintext.len();
    if padding_level > 0 {
        let target = googol_padding_tier(password.as_slice(), &salt, padding_level, original_len);
...
pub fn decrypt_file(input_path: &Path, output_path: &Path, password: ProtectedBuffer) -> Result<(), CryptoError> {
